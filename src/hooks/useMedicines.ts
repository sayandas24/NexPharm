...
import { MedicineBatchTable, MedicinesTable, PharmacyMedicineTable } from '@/types/database-types';
...
export function useMedicines(pharmacyId?: string) {
  ...
  const [medicines, setMedicines] = useState<(MedicinesTable | PharmacyMedicineWithDetails)[]>([]);
  ...
  try {
    ...
    if (pharmacyId) {
      ...
      const result = await db
        ...
        .innerJoin('medicines', 'medicines.id', 'pharmacy_medicines.medicine_id')
        .select([
      ...
      console.log('Pharmacy medicines result:', result);
      setMedicines(result as PharmacyMedicineTable[]);
    } else {
...
console.log(medicines)
  ...
  // mark Add medicine to pharmacy (junction table)
  const addMedicineToPharmacy = useCallback(async (
    medicineId: string,
    pharmacyId: string,
    inventoryData: {
      mrp: number;
  ...
  const createMedicine = useCallback(async (medicineData: Omit<MedicinesTable, 'id' | 'created_at' | 'updated_at'>) => {
  ...
  const fetchBatchesForMedicine = useCallback(async (medicineId: string, pharmacyId?: string) => {
  ...
  const searchMedicine = useCallback(async (
    searchTerm: string, 
  ...
  const searchMedicinesByName = useCallback(async (searchTerm: string, pharmacyId?: string) => {
    ...
    try {
      if (pharmacyId) {
        ...
        return result as PharmacyMedicineWithDetails[];
      } else {
        const result = await db
          .selectFrom('medicines')
          .selectAll()
  ...
  // Get batch by ID
  const getBatchById = useCallback(async (batchId: string) => {
  ...
  const updateMedicineStock = useCallback(async (
    medicineId: string,
    pharmacyId: string,
    newQuantity: number
  ) => {
